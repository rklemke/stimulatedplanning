 ...
 */
	protected static SelectionObject generateWeek2KnowledgeTest10() {
		...
		option = new SelectionOption(getUUID(), 
				"e. Yes, unless the key is completely random.", "Yes, unless the key is completely random.", "");
		option.setCorrect(false);
		sele.addOption(option);
...
Authenticity: Alice is indeed the person who sent the message
	...
	protected static SelectionObject generateWeek4KnowledgeTest5() {
		SelectionObject sele = new SelectionObject(getUUID(), 
				"Test", 
				"5. Suppose Bob receives an encrypted message, Encpk <sub>(bob)(m)</sub>, and the hash of the message, hash<sub>(m)</sub>. It seems to have come from Alice. What are all security guarantees that Bob has?", 
				"",
				true, true, true);
		sele.setType(SelectionObjectType.CLAN_SINGLE_TEST);
		sele.setPurpose(SelectionObjectPurpose.TEST);
...
Answer b and yahooogle signed the certificate.
	...
	protected static SelectionObject generateWeek4KnowledgeTest6() {
		SelectionObject sele = new SelectionObject(getUUID(), 
				"Test", 
				"b. Suppose you visit a website “yahooogle.com” and it claims to be a company started by Google and Yahoo. What information in the certificate would make you trust the website’s security?", 
				"",
				true, true, true);
		sele.setType(SelectionObjectType.CLAN_SINGLE_TEST);
		sele.setPurpose(SelectionObjectPurpose.TEST);
